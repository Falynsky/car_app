...
class CarListCubit extends Cubit<CarListState> {
  ...
  Future<void> initCarList() async {
    final Either<Failure, List<CarEntity>> either = await getAllCarsUseCase(NoParams());
    either.fold(
      (Failure failure) => emit(state.copyWith(isLoading: true, cars: <CarEntity>[])),
      (List<CarEntity> cars) => emit(state.copyWith(isLoading: false, cars: cars)),
    );
  }
}
